...
async def wake_space(space_id):
    host = f"https://{space_id.replace('/', '-').lower()}.hf.space"
    print(f"Waking up: {space_id}...")
    try:
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        try:
            ...
            if r.status_code != 200:
                print(f"  Failed join: {r.status_code}")
                return False
...
async def main():
    ...
    # 1. Trigger wakeup
    await asyncio.gather(*(wake_space(c) for c in clones))
    
    print("\nWaiting 20 seconds for Spaces to boot up...")
