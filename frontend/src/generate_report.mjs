...
// Function to suggest CSS declaration based on prop+hex+opacity
function suggest(prop, hex, opacity) {
...
for (const [key, locs] of usedMap) {
  ...
  if (!css.includes(selector)) {
    const used_in = locs.map(l => l.file + ':' + l.line);
    missingList.push({
      selector,
      used_in: used_in,
      suggestion: suggest(prop, hex, opacity)
    });
  }
}

const totalUsed = usedMap.size;
const missingCount = missingList.length;
const alreadyCovered = totalUsed - missingCount;

console.log('USED_BUT_NOT_OVERRIDDEN=', JSON.stringify(missingList, null, 2));
console.log('ALREADY_OVERRIDDEN=', alreadyCovered);
