...
import {
  fetchMe, saveEducationSettings, getEducationSettings, generateMaterial,
  listMaterials, deleteMaterial, uploadSubjectMaterial, listDocuments,
  updateTeachingMethods, redeemShadowWorkspace, listShadowActivities, joinClassByToken
} from "@/lib/api";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { BookOpen, Plus, Trash2, Loader2, BrainCircuit, CalendarDays, Bot, BookText, Upload, Camera, FileText, ArrowRight, Eye, X, CheckCircle2, AlertCircle, School } from "lucide-react";
...
export default function EducationSettings() {
  ...
  // Shadow Workspace & Class Joining States
  const [shadowActivities, setShadowActivities] = useState([]);
  const [shadowCode, setShadowCode] = useState("");
  const [redeemingShadow, setRedeemingShadow] = useState(false);
  const [classToken, setClassToken] = useState("");
  const [joiningClass, setJoiningClass] = useState(false);

  const loadShadowActivities = async () => {
    try {
      const res = await listShadowActivities();
      setShadowActivities(res.activities || []);
    } catch {}
  };
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (me?.role === "pelajar" && me?.institution_code) {
          try {
            const res = await listShadowActivities();
            setShadowActivities(res.activities || []);
          } catch {}
  ...
  const handleJoinClass = async (e) => {
    ...
    if (!token) {
      toast.error("Masukkan kode token kelas");
      return;
    }
    setJoiningClass(true);
    try {
      await joinClassByToken(token);
      toast.success("Berhasil bergabung ke kelas!");
      setClassToken("");
      const edu = await getEducationSettings();
      setSubjects(edu.subjects || []);
      setSchedule(edu.schedule || []);
      window.location.reload();
    } catch (err) {
      toast.error(err?.response?.data?.detail || "Gagal bergabung ke kelas. Periksa token Anda.");
    } finally {
      setJoiningClass(false);
    }
  };

  const handleRedeemShadow = async (e) => {
    e.preventDefault();
    const code = shadowCode.trim();
    if (!code) {
      toast.error("Masukkan kode redeem SaaS");
      return;
    }
    setRedeemingShadow(true);
    try {
      await redeemShadowWorkspace(code);
      toast.success("Kode redeem SaaS berhasil digunakan!");
      setShadowCode("");
      await loadShadowActivities();
    } catch (err) {
      toast.error(err?.response?.data?.detail || "Gagal redeem kode. Pastikan kode valid (contoh: SAAS-MATH-99).");
    } finally {
      setRedeemingShadow(false);
    }
  };
    ...
    <div className="w-full" data-testid="education-settings-page">
      ...
      {isInstitutionalStudent ? (
        <div className="space-y-6">
          ...
          <div className="bg-white border border-[#E2E0D8] rounded-xl p-6 relative overflow-hidden">
            <div className="absolute top-0 right-0 px-4 py-1.5 bg-[#2D6A4F] text-white text-[10px] font-bold uppercase tracking-wider rounded-bl-lg">
              Autopilot Aktif
            </div>
            <h2 className="font-heading text-lg text-[#1A1B26] flex items-center gap-2 mb-4">
              <School className="w-5 h-5 text-[#1D2D50]" />
              Detail Pembelajaran Institusi
            </h2>
            <div className="grid sm:grid-cols-2 gap-4 text-sm mt-3">
              <div className="p-4 bg-[#F8F6F0] rounded-xl border border-[#E2E0D8]/60">
                <div className="text-xs text-[#A0A2B1] uppercase tracking-wider">Sekolah</div>
                <div className="font-medium text-[#1A1B26] mt-1">{user?.institution || "-"}</div>
              </div>
              <div className="p-4 bg-[#F8F6F0] rounded-xl border border-[#E2E0D8]/60">
                <div className="text-xs text-[#A0A2B1] uppercase tracking-wider">Kelas Terdaftar</div>
                <div className="font-medium text-[#1A1B26] mt-1">{user?.enrolled_class || "-"}</div>
              </div>
              <div className="p-4 bg-[#F8F6F0] rounded-xl border border-[#E2E0D8]/60">
                <div className="text-xs text-[#A0A2B1] uppercase tracking-wider">Jurusan / Program Studi</div>
                <div className="font-medium text-[#1A1B26] mt-1">{user?.major || "Umum"}</div>
              </div>
              <div className="p-4 bg-[#F8F6F0] rounded-xl border border-[#E2E0D8]/60">
                <div className="text-xs text-[#A0A2B1] uppercase tracking-wider font-mono">Kode Token Kelas</div>
                <div className="font-mono font-medium text-[#1A1B26] mt-1">{user?.class_token_used || "Terhubung Langsung"}</div>
              </div>
            </div>
            <p className="text-xs text-[#646675] mt-4 leading-relaxed">
              Jadwal, mata pelajaran, dan modul ajar Anda diselaraskan secara langsung dengan portal pengajar sekolah Anda. Anda tidak perlu mengubah setelan ini secara manual.
            </p>
          </div>

          {/* Mata Pelajaran & Jadwal Pelajaran (Read-Only) */}
          <div className="grid md:grid-cols-2 gap-6">
            {/* Subjects List */}
            <div className="bg-white border border-[#E2E0D8] rounded-xl p-6">
              <h3 className="font-heading text-base text-[#1A1B26] flex items-center gap-2 mb-4">
                <BookText className="w-4 h-4 text-[#E5A93C]" />
                Mata Pelajaran Aktif
              </h3>
              {subjects.length === 0 ? (
                <div className="text-sm text-[#A0A2B1]">Belum ada mata pelajaran terdaftar.</div>
              ) : (
                <div className="flex flex-wrap gap-2">
                  {subjects.map((s) => (
                    <div key={s.id} className="px-3 py-1.5 bg-[#F8F6F0] border border-[#E2E0D8] rounded-lg text-sm text-[#1A1B26]">
                      {s.name}
                    </div>
                  ))}
                </div>
            ...
            <div className="bg-white border border-[#E2E0D8] rounded-xl p-6">
              <h3 className="font-heading text-base text-[#1A1B26] flex items-center gap-2 mb-4">
                <CalendarDays className="w-4 h-4 text-[#E5A93C]" />
                Jadwal Belajar
              </h3>
              {schedule.length === 0 ? (
                <div className="text-sm text-[#A0A2B1]">Belum ada jadwal yang disinkronisasikan.</div>
              ) : (
                <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                  {schedule.map((s, i) => {
                    const matchedSubj = subjects.find(sub => sub.id === s.subject_id);
                    return (
                      <div key={i} className="flex justify-between items-center p-2.5 bg-[#F8F6F0] rounded-lg text-xs text-[#1A1B26]">
                        <span className="font-semibold">{s.day}</span>
                        <span>{s.start_time} - {s.end_time}</span>
                        <span className="font-medium text-[#1D2D50] max-w-[120px] truncate">{matchedSubj?.name || "Mata Pelajaran"}</span>
                      </div>
                    );
                  })}
                </div>
